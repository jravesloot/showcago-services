...
Chicago venue show aggregation platform — collects events from 13+ venue sources, matches artists, and creates show records.

- `lib/showcago_services/schema/` — Ecto schemas: Venue, Show, Artist, VenueSource
- `lib/showcago_services/venues/` — Venue collection logic and Source behaviour
- `lib/showcago_services/venues/sources/` — Individual venue source modules (Ticketmaster API + HTML scrapers)
- `lib/showcago_services/workers/` — Oban background workers (daily collection dispatch + per-source collection)
- `lib/showcago_services/artists.ex` — Artist fuzzy matching via PostgreSQL `word_similarity()`
- `lib/showcago_services/shows.ex` — Show context (CRUD, queries)
- `lib/showcago_services/jido/` — Jido framework integration (AI actions, code agent)
- `lib/showcago_services_web/live/` — Admin LiveViews for venues, shows, artists, users
- `lib/showcago_services_web/controllers/` — HTTP controllers (pages, auth sessions)

Data flow: Oban cron dispatches per-source jobs → fetch raw payload from venue website/API → store in VenueSource → parse events → fuzzy-match artists → create Show records