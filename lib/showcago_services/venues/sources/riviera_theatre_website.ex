defmodule ShowcagoServices.Venues.Sources.RivieraTheatreWebsite do
  @moduledoc false
  ...
  @riviera_url "https://www.jamusa.com/venues/riviera-theatre"
  @payload_source "riviera_theatre_website"
  ...
  @impl true
  def venue_name, do: "Riviera Theatre"
  ...
  def collect_payload(_venue, opts) do
    fetch_events_fun =
      Keyword.get(opts, :fetch_events_fun, &fetch_riviera_events/0)

    case fetch_events_fun.() do
      {:ok, events} ->
        Logger.info("[venue_parser] riviera theatre website events fetched count=#{length(events)}")
      ...
      {:error, reason} ->
        Logger.warning("[venue_parser] riviera theatre website fetch failed reason=#{inspect(reason)}")
  ...
  defp fetch_riviera_events do
    case Req.get(url: @riviera_url, headers: [{"user-agent", "ShowcagoServices/1.0"}]) do
      {:ok, %Req.Response{status: status, body: body}}
      when status in 200..299 and is_binary(body) ->
        {:ok, parse_events_from_html(body)}

      {:ok, %Req.Response{status: status}} ->
        Logger.warning("[venue_parser] riviera theatre website non-200 status=#{status}")
        {:error, {:unexpected_status, status}}

      {:error, reason} ->
        Logger.warning("[venue_parser] riviera theatre website request failed reason=#{inspect(reason)}")
  ...
  def parse_events_from_html(html) when is_binary(html) do
    ...
    error ->
      Logger.warning("[venue_parser] riviera theatre website parse failed reason=#{inspect(error)}")
      []
  end
