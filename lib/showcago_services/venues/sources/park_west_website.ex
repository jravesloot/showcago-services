defmodule ShowcagoServices.Venues.Sources.ParkWestWebsite do
  @moduledoc false
  ...
  @park_west_url "https://www.jamusa.com/venues/park-west"
  @payload_source "park_west_website"
  ...
  def collect_payload(_venue, opts) do
    fetch_events_fun =
      Keyword.get(opts, :fetch_events_fun, &fetch_park_west_events/0)

    case fetch_events_fun.() do
      {:ok, events} ->
        Logger.info("[venue_parser] park west website events fetched count=#{length(events)}")
      ...
      {:error, reason} ->
        Logger.warning("[venue_parser] park west website fetch failed reason=#{inspect(reason)}")
  ...
  defp fetch_park_west_events do
    case Req.get(url: @park_west_url, headers: [{"user-agent", "ShowcagoServices/1.0"}]) do
      {:ok, %Req.Response{status: status, body: body}}
      when status in 200..299 and is_binary(body) ->
        {:ok, parse_events_from_html(body)}

      {:ok, %Req.Response{status: status}} ->
        Logger.warning("[venue_parser] park west website non-200 status=#{status}")
        {:error, {:unexpected_status, status}}

      {:error, reason} ->
        Logger.warning("[venue_parser] park west website request failed reason=#{inspect(reason)}")
  ...
  def parse_events_from_html(html) when is_binary(html) do
    ...
    error ->
      Logger.warning("[venue_parser] park west website parse failed reason=#{inspect(error)}")
      []
  end
  ...
  defp extract_event_date(block) do
    # The date div has an aria-label with the full date including year
    # e.g., aria-label="March 27 2026" or aria-label="April  3 2026"
  ...
  def parse_date_label(date_text) when is_binary(date_text) do
    # Handles "Month DD YYYY" format (e.g., "March 27 2026", "April  3 2026")
