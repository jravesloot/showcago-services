defmodule ShowcagoServices.Venues.Sources.AragonBallroomTicketmaster do
  @moduledoc false
  ...
  @impl true
  def source_key, do: "aragon_ballroom_ticketmaster"

  @impl true
  def venue_name, do: "Aragon Ballroom"
  ...
  def collect_payload(_venue, opts) do
    fetch_ticketmaster_events_fun =
      Keyword.get(
        opts,
        :fetch_ticketmaster_events_fun,
        &fetch_aragon_ballroom_ticketmaster_events/0
      )

    case fetch_ticketmaster_events_fun.() do
      {:ok, tm_events} ->
        Logger.info("[venue_parser] aragon ballroom ticketmaster events fetched count=#{length(tm_events)}")

        {:ok,
         Jason.encode!(%{
           "source" => "aragon_ballroom_ticketmaster_api",
           "fetched_at" => :second |> DateTime.utc_now() |> DateTime.to_iso8601(),
           "events" => tm_events
         })}

      {:error, reason} ->
        Logger.warning("[venue_parser] aragon ballroom ticketmaster fetch failed reason=#{inspect(reason)}")
  ...
  def extract_events(payload) when is_binary(payload) do
    case Jason.decode(payload) do
      {:ok, %{"source" => "aragon_ballroom_ticketmaster_api", "events" => events}}
      when is_list(events) ->
        events
  ...
  defp fetch_aragon_ballroom_ticketmaster_events do
    events =
      @aragon_ballroom_tm_urls
      |> Enum.with_index()
      |> Enum.flat_map(fn {url, idx} ->
        ...
        case Req.get(url: url) do
          ...
          {:ok, %Req.Response{status: status}} ->
            Logger.warning("[venue_parser] aragon ballroom ticketmaster non-200 status=#{status} url=#{url}")

            []

          {:error, reason} ->
            Logger.warning(
              "[venue_parser] aragon ballroom ticketmaster request failed reason=#{inspect(reason)} url=#{url}"
            )
