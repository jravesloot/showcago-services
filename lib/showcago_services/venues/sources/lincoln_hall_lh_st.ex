defmodule ShowcagoServices.Venues.Sources.LincolnHallLhSt do
  @moduledoc false
  ...
  @venue_post_title "Lincoln Hall"
  @pages_to_fetch 2

  @impl true
  def source_key, do: "lincoln_hall_lh_st"

  @impl true
  def venue_name, do: "Lincoln Hall"
  ...
  def collect_payload(_venue, opts) do
    fetch_events_fun =
      Keyword.get(opts, :fetch_events_fun, &fetch_lincoln_hall_events/0)

    case fetch_events_fun.() do
      {:ok, events} ->
        Logger.info("[venue_parser] lincoln hall lh-st events fetched count=#{length(events)}")

        {:ok,
         Jason.encode!(%{
           "source" => "lincoln_hall_lh_st_api",
           "fetched_at" => :second |> DateTime.utc_now() |> DateTime.to_iso8601(),
           "events" => events
         })}

      {:error, reason} ->
        Logger.warning("[venue_parser] lincoln hall lh-st fetch failed reason=#{inspect(reason)}")
  ...
  def extract_events(payload) when is_binary(payload) do
    case Jason.decode(payload) do
      {:ok, %{"source" => "lincoln_hall_lh_st_api", "events" => events}}
      when is_list(events) ->
        events
  ...
  defp fetch_lincoln_hall_events do
    events =
      1..@pages_to_fetch
      |> Enum.flat_map(&fetch_page/1)
      |> Enum.filter(&lincoln_hall_show?/1)
  ...
  defp fetch_page(page) do
    ...
    case Req.get(url: url, headers: [{"user-agent", "ShowcagoServices/1.0"}]) do
      ...
      {:ok, %Req.Response{status: status}} ->
        Logger.warning("[venue_parser] lincoln hall lh-st non-200 status=#{status} page=#{page}")

        []

      {:error, reason} ->
        Logger.warning("[venue_parser] lincoln hall lh-st request failed reason=#{inspect(reason)} page=#{page}")

        []
    end
  end

  defp lincoln_hall_show?(show) when is_map(show) do
    get_in(show, ["acf", "venue", "post_title"]) == @venue_post_title
  end

  defp lincoln_hall_show?(_), do: false
