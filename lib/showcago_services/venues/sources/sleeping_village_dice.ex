defmodule ShowcagoServices.Venues.Sources.SleepingVillageDice do
  @moduledoc false
  ...
  @venue_name "Sleeping Village"

  @impl true
  def source_key, do: "sleeping_village_dice"
  ...
  def collect_payload(_venue, opts) do
    ...
    case fetch_events_fun.() do
      {:ok, events} ->
        Logger.info("[venue_parser] sleeping village dice events fetched count=#{length(events)}")
      ...
      {:error, reason} ->
        Logger.warning("[venue_parser] sleeping village dice fetch failed reason=#{inspect(reason)}")
  ...
  def extract_events(payload) when is_binary(payload) do
    case Jason.decode(payload) do
      {:ok, %{"source" => "sleeping_village_dice_api", "events" => events}}
      when is_list(events) ->
        events
  ...
  defp fetch_dice_events do
    ...
    case Req.get(url: url, headers: [{"x-api-key", @dice_api_key}]) do
      ...
      {:ok, %Req.Response{status: status}} ->
        Logger.warning("[venue_parser] sleeping village dice non-200 status=#{status}")

        {:error, {:unexpected_status, status}}

      {:error, reason} ->
        Logger.warning("[venue_parser] sleeping village dice request failed reason=#{inspect(reason)}")
