defmodule ShowcagoServices.Venues.Sources.BeatKitchenSeeTickets do
  @moduledoc false
  ...
  @beat_kitchen_url "https://www.beatkitchen.com"
  ...
  @impl true
  def source_key, do: "beat_kitchen_seetickets"

  @impl true
  def venue_name, do: "Beat Kitchen"
  ...
  def collect_payload(_venue, opts) do
    fetch_events_fun =
      Keyword.get(opts, :fetch_events_fun, &fetch_beat_kitchen_events/0)

    case fetch_events_fun.() do
      {:ok, events} ->
        Logger.info("[venue_parser] beat kitchen seetickets events fetched count=#{length(events)}")
      ...
      {:error, reason} ->
        Logger.warning("[venue_parser] beat kitchen seetickets fetch failed reason=#{inspect(reason)}")
  ...
  def extract_events(payload) when is_binary(payload) do
    case Jason.decode(payload) do
      {:ok, %{"source" => "beat_kitchen_seetickets", "events" => events}}
      when is_list(events) ->
        events
  ...
  defp fetch_beat_kitchen_events do
    case Req.get(url: @beat_kitchen_url, headers: [{"user-agent", "ShowcagoServices/1.0"}]) do
      ...
      {:ok, %Req.Response{status: status}} ->
        Logger.warning("[venue_parser] beat kitchen seetickets non-200 status=#{status}")

        {:error, {:unexpected_status, status}}

      {:error, reason} ->
        Logger.warning("[venue_parser] beat kitchen seetickets request failed reason=#{inspect(reason)}")
  ...
  def parse_events_from_html(html) when is_binary(html) do
    # Split the HTML by event container boundaries, then parse each block
    html
    |> String.split("seetickets-list-event-container")
    # Drop the first chunk (before the first event)
    |> Enum.drop(1)
  ...
  def parse_date_text(date_text) do
    # Handle "Day Mon DD" format (e.g., "Sat Mar 07")
    case Regex.run(~r/\w+\s+(\w+)\s+(\d+)/, date_text) do
      ...
      _ ->
        # Handle multi-day format like "May 22 - May 23" — use the start date
  ...
  # Infer year: if the date would be more than 60 days in the past,
